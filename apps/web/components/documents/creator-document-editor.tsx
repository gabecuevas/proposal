...
type CreatorDocumentEditorProps = {
  documentId: string;
};

export function CreatorDocumentEditor({ documentId: documentIdProp }: CreatorDocumentEditorProps) {
  ...
  useEffect(() => {
    ...
    async function load() {
      await Promise.all([loadDocument(documentIdProp), loadContacts()]);
    }
    void load();
